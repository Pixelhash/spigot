...
[[ ! -z "${SERVER_ROOT}" ]]  && declare -r SERVER_ROOT=${SERVER_ROOT}   || SERVER_ROOT="/srv/spigot"
[[ ! -z "${BACKUP_DEST}" ]]  && declare -r BACKUP_DEST=${BACKUP_DEST}   || BACKUP_DEST="/srv/spigot/backup"
[[ ! -z "${BACKUP_PATHS}" ]] && declare -r BACKUP_PATHS=${BACKUP_PATHS} || BACKUP_PATHS="."
[[ ! -z "${KEEP_BACKUPS}" ]] && declare -r KEEP_BACKUPS=${KEEP_BACKUPS} || KEEP_BACKUPS="10"
[[ ! -z "${GAME_USER}" ]]    && declare -r GAME_USER=${GAME_USER}       || GAME_USER="spigot"
...
[[ ! -z "${SERVER_START_CMD}" ]] && declare -r SERVER_START_CMD=${SERVER_START_CMD} || SERVER_START_CMD="java -Xms512M -Xmx1024M -DIReallyKnowWhatIAmDoingISwear=true -jar './${MAIN_EXECUTABLE}' -o true nogui"
...
source /etc/default/"${game}" 2>/dev/null || >&2 echo "Could not source /etc/default/${game}"
...
backup_files() {
	...
	if ${SUDO_CMD} screen -S "${SESSION_NAME}" -Q select . > /dev/null; then
		echo "Please stop the server or use 'spigot restart -b' to backup the server!"
		exit 0
		game_command save-off
		game_command save-all
		sync && wait
		${SUDO_CMD} tar -C "${SERVER_ROOT}" -czf "${BACKUP_DEST}/${FILE}" --exclude='spigot-1.12.2.jar' --exclude='./backup' --totals . 2>&1 | grep -v "tar: Removing leading "
...
help() {
	...
	Usage: ${myname} {start|stop|status|backup|restore|command <command>|console}
	    ...
	    restart [-b]         Restart the ${game} server (-b with backup)
